...
public static class StringExtensions
{
    ...
    public static string FormatWithParams(this string format, object source)
    {
        return FormatWithParams(format, null, source);
    }

    public static string FormatWithParams(this string format, IFormatProvider provider, object source)
    {
        if (format == null)
        ...
        string rewrittenFormat = r.Replace(format, delegate(Match m)
        {
            ...
            {
                return String.Format("Invalid property: {0}", propertyGroup);
            }
        });
    ...
    public static Type GetFullyQualifiedType(this string typeName) {
    ...
    public static string CamelCaseToWords(this Enum input) {
        return Regex.Replace(input.ToString().FirstCharToUpper(), "([a-z](?=[A-Z])|[A-Z](?=[A-Z][a-z]))", "$1 ");
    }
