...
public static class DateTimeExtensions
{
    ...
    public class MinutesWorked
    {
        ...
        public double Hours {
            get {
                return Math.Round((Minutes / 60) + ((double)(Minutes % 60) / 60),2);
            }
        }
    }

    public static string WeekOfYear(this DateTime dateTimeValue, DayOfWeek StartDayOfWeek)
    {
        CultureInfo cul = CultureInfo.CurrentCulture;
        _ = cul.Calendar.GetWeekOfYear(dateTimeValue, CalendarWeekRule.FirstDay, StartDayOfWeek);

        int weekNum = cul.Calendar.GetWeekOfYear(dateTimeValue, CalendarWeekRule.FirstDay, StartDayOfWeek);
        _ = weekNum == 52 && dateTimeValue.Month == 1 ? dateTimeValue.Year - 1 : dateTimeValue.Year;
        return string.Format("Week: {0}", weekNum);
    }

    public static string YearOfWeek(this DateTime dateTimeValue, DayOfWeek StartDayOfWeek)
    {
        CultureInfo cul = CultureInfo.CurrentCulture;
        _ = cul.Calendar.GetWeekOfYear(dateTimeValue, CalendarWeekRule.FirstDay, StartDayOfWeek);
        ...
        int year = weekNum == 52 && dateTimeValue.Month == 1 ? dateTimeValue.Year - 1 : dateTimeValue.Year;
        return year.ToString();
    }

    public static IEnumerable<MinutesWorked> SplitMinutesToDays(this DateTime endValue, DateTime StartDateTime)
    {
        List<MinutesWorked> minutesWorkedList = new List<MinutesWorked>();
        MinutesWorked minutesWorked = null;

        while (StartDateTime.AddMinutes(1) <= endValue)
        {
            if (minutesWorked == null)
            {
                minutesWorked = new MinutesWorked() { DayOfWeek = StartDateTime.DayOfWeek, Minutes = 1, PublicHoliday = DateSystem.IsPublicHoliday(StartDateTime,CountryCode.ZA)  };
                minutesWorkedList.Add(minutesWorked);
            }
            else if (minutesWorked.DayOfWeek == StartDateTime.DayOfWeek)
                minutesWorked.Minutes += 1;
            else
            {
                minutesWorked = new MinutesWorked() { DayOfWeek = StartDateTime.DayOfWeek, Minutes = 1, PublicHoliday = DateSystem.IsPublicHoliday(StartDateTime, CountryCode.ZA) };
                minutesWorkedList.Add(minutesWorked);
        ...
        return minutesWorkedList;
    }

    ///<summary>
    ///	Return System South African UTC DateTime
    ///</summary>
    public static DateTime ConvertToCurrentTimeZone(this DateTime value, string CurrentTimeZone) {
        var myTimeZone = TimeZoneInfo.FindSystemTimeZoneById(CurrentTimeZone);
        return TimeZoneInfo.ConvertTimeFromUtc(value, myTimeZone);
    }

    ///<summary>
    ///	 Invariant Culture format
    ///</summary>
    public static string ConvertToInvariantCultureFormat(this DateTime value) {
        IFormatProvider fProvider = (IFormatProvider)CultureInfo.InvariantCulture.GetFormat(typeof(DateTimeFormatInfo));
        return value.ToString(fProvider);
    }
    ...
    public static DateTime GetFirstDayOfLastMonth(this DateTime date)
    {
        return GetFirstDayOfMonth(date).AddMonths(-1);
    }

    //Returns the last day of last month
    public static DateTime GetLastDayOfLastMonth(this DateTime date)
    {
        return GetLastDayOfMonth(date).AddMonths(-1);
    }
    ...
    public static int GetQuarterNumeric(this DateTime dateTime) {
        if (dateTime.Month >= 1 && dateTime.Month <= 3)
            return 1;
        else if (dateTime.Month >= 4 && dateTime.Month <= 6)
            return 2;
